import { forwardRef, ComponentRef, useState, ReactElement, ComponentProps, ForwardedRef, useRef, useLayoutEffect } from 'react';
...
export const Select = forwardRef(function Select<T extends string | number>(
  ...
  useLayoutEffect(() => {
    if (anchorRef.current) { // Platform.OS !== 'web' の条件を削除
      anchorRef.current.measure((_x, _y, width) => {
        setMenuWidth(width);
      });
    }
  }, [visible]); // visibleが変わったとき（Menuが開かれるとき）に幅を再計算
